...
def generate_seo_metadata(story_text, image_files, prompt_text):
    ...
    try:
        ...
        print("u2705 Initializing SEO metadata generator client...")
    except Exception as e:
        print(f"ud83dudd34 Error initializing SEO metadata generator client: {e}")
    ...
    print("u23f3 Generating SEO-friendly metadata...")
        ...
        if response.candidates and response.candidates[0].content:
            ...
            try:
                ...
                if not all(key in metadata for key in ['title', 'description', 'tags']):
                    print("u26a0ufe0f Metadata is missing required fields, using fallback...")
                    return default_seo_metadata(story_text, prompt_text)

                print("u2705 SEO metadata generated successfully")
                return metadata
            except json.JSONDecodeError:
                print("u26a0ufe0f Failed to parse metadata as JSON, using fallback...")
                return default_seo_metadata(story_text, prompt_text)
    except Exception as e:
        print(f"u26a0ufe0f Error generating SEO metadata: {e}")
        return default_seo_metadata(story_text, prompt_text)
...
def default_seo_metadata(story_text, prompt_text):
    ...
    print("u2705 Created default SEO metadata")
    return {
        "title": title,
