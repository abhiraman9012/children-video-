...
def generate_prompt(prompt_input="Create a children's story with a different animal character and a unique adventure theme. Be creative with the setting and storyline.", use_streaming=True):
    ...
    try:
        ...
        print("u2705 Initializing prompt generator client using genai.Client...")
    except Exception as e:
        print(f"ud83dudd34 Error initializing prompt generator client: {e}")
        return None
    ...
    try:
        if use_streaming:
            print("u23f3 Generating prompt via streaming API...")
            ...
            for chunk in stream:
                try:
                    ...
                    print(f"u26a0ufe0f Error processing prompt chunk: {e}")
                    continue
        else:
            print("u23f3 Generating prompt via non-streaming API...")
        ...
        if not generated_prompt.startswith("Generate a story about"):
            ...
            print("u26a0ufe0f Generated prompt did not have correct structure, applying formatting fix")
        ...
        print("\nu2705 Prompt generation complete.")
        print(f"Final generated prompt: {generated_prompt}")
        return generated_prompt

    except Exception as e:
        print(f"ud83dudd34 Error generating prompt: {e}")
        return None
